...
class Helmholtz(nn.Module):
    def __init__(self, dx, k, eps, mu_p, sigma_p, mu_k, sigma_k):
        super().__init__()
        # returns all components of Helmholtz & Poisson residual
    ...
    def laplacian(self, u):
        # ∇²u = ∂²u/∂x² + ∂²u/∂y²
        return self.d2udx2(u) + self.d2udy2(u)
    ...
    def drdp(self, x):
        ...
        r, d2pdx2, d2pdy2 = self.r_diff(p, k)

        d2pdx2_dp = self.d_du('d2udx2', d2pdx2, p)
        d2pdy2_dp = self.d_du('d2udy2', d2pdy2, p)

        dr_dp = 2*r*(d2pdx2_dp + d2pdy2_dp + self.k ** 2)
        return dr_dp
    ...
    def r_diff(self, p, k):
        ...
        d2pdx2 = self.d2udx2(p)
        d2pdy2 = self.d2udy2(p)
        residual = d2pdx2 + d2pdy2 + self.k ** 2 * p - k
        return residual, d2pdx2, d2pdy2
...
class NavierStokes(nn.Module):
    def __init__(self, dx, eps, mu_p, sigma_p, mu_k, sigma_k):
        super().__init__()
        # returns all components of Navier-Stokes residual
    ...
    def drdp(self, x):
        ...
        r, dkdx, dpdy = self.r_diff(p, k)

        dkdx_dk = self.d_du('dudx', dkdx, k)
        dpdy_dp = self.d_du('dudy', dpdy, p)

        dr_dp = 2*r*(dkdx_dk + dpdy_dp)
        return dr_dp
