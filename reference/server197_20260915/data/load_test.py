...
# from load_sensors import coefRandom, solRandom, sensorRandom
...
class PDEloader:
    ...
    def _darcy_load(self, data_path):
        dataset = {}
        file_path = data_path
        with h5py.File(file_path, 'r') as file:
            a = file['thresh_a_data'][:] # type: ignore
            u = file['thresh_p_data'][:] # type: ignore
        dataset[0] = np.stack([a, u], axis=0).transpose(3, 0, 1, 2) # type: ignore

        data = torch.tensor(np.concatenate(list(dataset.values()), axis=0)).to(torch.float32)
        label = torch.zeros((len(data), 1), dtype=torch.float32)
        return data, label

    def _poisson_load(self, data_path):
        dataset = {}
        file_path = data_path
        f = scipy.io.loadmat(file_path)['f_data']
        phi = scipy.io.loadmat(file_path)['phi_data']
        dataset[0] = np.stack([f, phi], axis=1)

        data = torch.tensor(np.concatenate(list(dataset.values()), axis=0)).to(torch.float32)
        label = torch.zeros((len(data), 1), dtype=torch.float32) + 1
        return data, label
    
    def _helmholtz_load(self, data_path):
        dataset = {}

        file_path = data_path
        f = scipy.io.loadmat(file_path)['f_data']
        psi = scipy.io.loadmat(file_path)['psi_data']
        dataset[0] = np.stack([f, psi], axis=1)

        data = torch.tensor(np.concatenate(list(dataset.values()), axis=0)).to(torch.float32)
        label = torch.zeros((len(data), 1), dtype=torch.float32) + 2
        return data, label

    def _nsnonbounded_load(self, data_path):
        dataset = {}
        file_path = data_path
        with h5py.File(file_path, 'r') as file:
            a = file['w0'][:] # type: ignore
            u = file["w"][:, :, :, -1] # type: ignore
        dataset[0] = np.stack([a, u], axis=1) # type: ignore

        data = torch.tensor(np.concatenate(list(dataset.values()), axis=0)).to(torch.float32)
        label = torch.zeros((len(data), 1), dtype=torch.float32) + 3
        return data, label
    
    def _burger_load(self, data_path):
        dataset = {}
        ...
        data = torch.tensor(np.concatenate(list(dataset.values()), axis=0)).to(torch.float32)
        label = torch.zeros((len(data), 1), dtype=torch.float32) + 4
        return data, label
    ...
    def _reaction_diffusion_load(self, data_path):
        ...
        with h5py.File(file_path, "r") as f:
            for k in tqdm(list(f.keys())):
                ...
                u = np.expand_dims(f[k]['data'][50, :, :, 0], axis = 0) # type: ignore
                v = np.expand_dims(f[k]['data'][50, :, :, 1], axis = 0) # type: ignore
                dataset[k] = np.stack([u0, v0, u, v], axis = 1)
        
        data = torch.tensor(np.concatenate(list(dataset.values()), axis=0)).to(torch.float32)
        label = torch.zeros((len(data), 1), dtype=torch.float32) + 5
        return data, label
    ...
    def _shallow_water_load(self, data_path):
        ...
        data = torch.tensor(np.concatenate(list(dataset.values()), axis=0)).to(torch.float32)
        label = torch.zeros((len(data), 1), dtype=torch.float32) + 6
        return data, label
