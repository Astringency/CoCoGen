"""Checkpoint-compatible network from Christian Jacobsen's CoCoGen.

Based on the archived server197 implementation; architecture and conditioning
semantics intentionally preserved. Only imports/configuration access are isolated.
"""
import torch
...
class UNET1(nn.Module):
    def __init__(self, in_channels, n_feat = 256, pool_size=4, data_size=64, cond_size=16, controlnet=None):
        ...
        if self.controlnet:
            self.control_condition_encoder = CondVec2Img(**controlnet["condition_encoder"]["params"]) #CondVec2Img(cond_size, data_size, in_channels)
        ...
        self.up0 = nn.Sequential(
            ...
            nn.ConvTranspose2d(2 * n_feat, 2 * n_feat, kernel_size=pool_size, stride=pool_size), # otherwise just have 2*n_feat
